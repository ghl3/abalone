"""Tests for `model.encoder`: D6 symmetry math + plane/policy permutation.
...
regression surfaces immediately:
  ...
  * Plane permutation: identity is no-op; centre marble is invariant;
    a marble at A1 maps under R to A5 as the cell math predicts.
...
class TestPlanePerm:
    def _empty_planes(self) -> np.ndarray:
        return np.zeros((6, 9, 9), dtype=np.float32)

    def test_identity_is_noop(self):
        rng = np.random.default_rng(0)
        planes = rng.random((6, 9, 9), dtype=np.float32) * VALID_CELL_MASK
    ...
    def test_centre_marble_is_invariant(self):
        ...
        planes[0] = bb_to_plane(1 << cell("E5"))
        for s in range(NUM_SYMS):
            out = apply_sym_to_planes(planes, s)
            np.testing.assert_array_equal(
                out[0], planes[0], f"sym {s} should not move the E5 marble"
            )

    def test_A1_marble_lands_at_A5_under_R(self):
        planes = self._empty_planes()
        planes[0] = bb_to_plane(1 << cell("A1"))
    ...
    def test_constant_planes_are_invariant_on_board(self):
        # Channels 2–5 (captures, ply, valid mask) are constants broadcast
        # across the board. Sym is defined only over the 61 valid cells,
        # so the test asserts invariance restricted to the on-board mask
        # — off-board entries in the output are zeroed by the sym op
        # regardless of input, which is fine because the network has the
        # valid-mask channel and never relies on off-board values.
        planes = self._empty_planes()
        planes[2].fill(0.5)
        planes[3].fill(0.25)
        planes[4].fill(0.1)
    ...
    def test_marble_count_is_preserved(self):
        ...
        bb = int(rng.integers(0, 1 << 60)) & ((1 << 81) - 1)
        planes = self._empty_planes()
        planes[0] = bb_to_plane(bb)
        before = int((planes[0] * VALID_CELL_MASK).sum())
        for s in range(NUM_SYMS):
            out = apply_sym_to_planes(planes, s)
            after = int((out[0] * VALID_CELL_MASK).sum())
